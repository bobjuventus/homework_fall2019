...
class DQNCritic(BaseCritic):
    ...
    def _build(self, q_func):
        ...
        # self.q_t_values: all the Q(s,a) values at step t
        # sefl.q_t: given actual action a, its Q value Q(s,a)
        ...
        # q_tp1_values: all the Q_target(s',a') values at step t but next timestep
        q_tp1_values = q_func(self.obs_tp1_ph, self.ac_dim, scope='target_q_func', reuse=False)

        # q_tp1: pick the action that gives the largest Q value of the next timestep
        if self.double_q:
            ...
            # TODO
            # Copy parameters of original Q function at next state s', to get all Q(s',a')
            temp_q = q_func(self.obs_tp1_ph, self.ac_dim, scope='q_func', reuse=True)
            # from all the Q(s',a') pick a' that has the largest Q(s',a')
            action = tf.compat.v1.argmax(temp_q, axis=1)
            # Calculate new Q-value with Q_target(s',a')
            q_tp1 = tf.reduce_sum(q_tp1_values * tf.one_hot(action, self.ac_dim), axis=1)
        else:
        ...
        q_func_vars = tf.compat.v1.get_collection(tf.compat.v1.GraphKeys.TRAINABLE_VARIABLES, scope="q_func")
        target_q_func_vars = tf.compat.v1.get_collection(tf.compat.v1.GraphKeys.TRAINABLE_VARIABLES, scope="target_q_func")
