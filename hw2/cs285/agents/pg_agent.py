...
class PGAgent(BaseAgent):
    ...
    def _discounted_return(self, rewards):
        ...
        discounts = [self.gamma**(indices[i]) for i in range(T)]

        # 3) create a list where the entry at each index (t') is gamma^(t') * r_{t'}
        discounted_rewards = [discounts[i]*rewards[i] for i in range(T)]
    ...
    def _discounted_cumsum(self, rewards):
        ...
        for start_time_index in range(len(rewards)): 
            ...
            indices = [(start_time_index + i) for i in range(T-start_time_index)]

            # 2) create a list where the entry at each index (t') is gamma^(t'-t)
            discounts = [self.gamma**(indices[i] - start_time_index) for i in range(T-start_time_index)]
            ...
            discounted_rtg = [discounts[i]*rewards[start_time_index+i] for i in range(T-start_time_index)]
